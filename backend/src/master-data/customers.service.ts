...
export class CustomersService {
    ...
    try {
      ...
      if (existing) {
        throw new ConflictException(
          "Customer with this business number already exists.",
        )
      }
  ...
  async reassignSalesRep(
    ...
    try {
      ...
      if (customer.salesRepId === newSalesRepId) {
        throw new ConflictException(
          "Customer is already assigned to this Sales Rep.",
        )
      }
      ...
      const activeHistory = await queryRunner.manager.findOne(
      ...
      const audit = queryRunner.manager.create(AuditLog, {
        ...
        entityId: customer.id,
        changes: {
          previous: { salesRepId: oldSalesRepId },
          new: { salesRepId: newSalesRepId },
        },
      })
      await queryRunner.manager.save(audit)
  ...
  async updateCustomer(
    ...
    await this.customerRepo.update(id, updates)
    return this.customerRepo.findOne({ where: { id } })
  }

  /**
   * Approves a pending customer.
   */
  async approveCustomer(id: string): Promise<Customer> {
    await this.customerRepo.update(id, { status: "active" })
    return this.customerRepo.findOne({ where: { id } })
  }

  /**
   * Rejects a pending customer.
   */
  async rejectCustomer(id: string, reason: string): Promise<Customer> {
    await this.customerRepo.update(id, { status: "rejected", notes: reason })
    return this.customerRepo.findOne({ where: { id } })
  }
  ...
  async deactivateCustomer(id: string): Promise<Customer> {
    await this.customerRepo.update(id, { status: "inactive", active: false })
    return this.customerRepo.findOne({ where: { id } })
  }
}
