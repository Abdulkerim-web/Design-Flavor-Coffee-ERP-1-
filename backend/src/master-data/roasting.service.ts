    ...
    try {
      ...
      if (batch.status !== "in-progress") {
        throw new BadRequestException(
          "Only IN_PROGRESS batches can be completed or failed.",
        )
      }
      ...
      if (
        !greenStock ||
        Number(greenStock.available) < Number(batch.greenInputQuantity)
      ) {
        throw new BadRequestException("STOCK_INSUFFICIENT")
      }

      greenStock.onHand =
        Number(greenStock.onHand) - Number(batch.greenInputQuantity)
      greenStock.available =
        Number(greenStock.available) - Number(batch.greenInputQuantity)
  ...
  async confirmGreenReturn(
    ...
    try {
      ...
      if (roasterDeclaredReturnQty !== storekeeperConfirmedQty) {
        ...
        return {
          success: false,
          message: "Discrepancy logged for manager review.",
          discrepancy,
        }
      }
      ...
      if (greenStock) {
        ...
        greenStock.available =
          Number(greenStock.available) + storekeeperConfirmedQty
        await queryRunner.manager.save(greenStock)
      ...
      return {
        success: true,
        message: "Return confirmed and inventory updated.",
      }
    } catch (err) {
      await queryRunner.rollbackTransaction()
      throw err
