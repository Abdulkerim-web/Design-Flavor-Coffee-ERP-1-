...
describe('DeliveryService', () => {
  ...
  it('should block non-managers from verifying delivery', async () => {
    await expect(service.verifyDelivery('DEL-1', 'DRIVER-1', 'DRIVER')).rejects.toThrow(ForbiddenException);
    await expect(service.verifyDelivery('DEL-1', 'DRIVER-1', 'DRIVER')).rejects.toThrow('Only a manager can verify');
  });
  ...
  it('should allow manager to verify and start 7-day payment clock if missing', async () => {
    ...
    const diffDays = (mockDelivery.order.paymentDeadlineAt.getTime() - new Date().getTime()) / (1000 * 3600 * 24);
