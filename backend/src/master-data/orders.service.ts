...
export class OrdersService {
  ...
  async editPendingOrder(
    ...
    try {
      ...
      if (order.status !== 'draft' && order.status !== 'PENDING_MANAGER_CONFIRMATION') {
        throw new BadRequestException('Cannot edit order once it has been confirmed or moved past pending.');
      }
  ...
  async cancelOrder(orderId: string, userId: string) {
    ...
    try {
      ...
      if (postRoastingStatuses.includes(order.status)) {
        throw new ConflictException('Cannot cancel order after roasting has started. Production history must be preserved.');
      }
