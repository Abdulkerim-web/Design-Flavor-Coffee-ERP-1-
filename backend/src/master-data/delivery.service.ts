...
export class DeliveryService {
  ...
  async verifyDelivery(
    ...
    if (userRole !== 'MANAGER' && userRole !== 'GM') {
      throw new ForbiddenException('Only a manager can verify customer acceptance.');
    }
    ...
    try {
      ...
      if (delivery.status !== 'pending-customer-confirmation' && delivery.status !== 'delivered-proof-submitted') {
