...
export class DiscrepancyService {
  ...
  async resolveDiscrepancy(
    ...
    try {
      ...
      if (resolutionType === 'ACCEPT_EXPECTED') {
        ...
        if (customQuantity === undefined) throw new BadRequestException('Custom quantity required');
        if (!resolutionNote) throw new BadRequestException('Resolution note required for custom override');
      ...
      if (discrepancy.entityType === 'RoastingYield') {
        const batch = await queryRunner.manager.findOne(RoastingBatch, { where: { id: discrepancy.entityId }, relations: ['orderItem'] });
        if (batch) {
  ...
  async confirmRoastedYield(
    ...
    try {
      ...
      const existingTx = await queryRunner.manager.findOne(InventoryTransaction, {
        where: { referenceEntityType: 'RoastingBatch', referenceEntityId: batchId },
      });

      if (existingTx) {
        // Idempotent return: just return success if already processed
        await queryRunner.rollbackTransaction();
        return { success: true, message: 'Already confirmed', transaction: existingTx };
      }
      ...
      if (batch.status !== 'completed') throw new BadRequestException('Batch is not completed');

      // Check mismatch (0.001 logic)
      if (Number(storekeeperActualQty) !== Number(batch.expectedRoastedQuantity)) {
        // Discrepancy!
        ...
        await queryRunner.manager.save(discrepancy);
        await queryRunner.commitTransaction();
        return { success: false, message: 'Discrepancy recorded for manager review.' };
      }
