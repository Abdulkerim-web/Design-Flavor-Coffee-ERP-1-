...
describe('FeasibilityEngineService', () => {
  ...
  it('should prevent concurrent reservations from causing negative balance (throws STOCK_INSUFFICIENT)', async () => {
    ...
    await expect(service.reserveStock('ITEM-1', 'PROD-1', 15, 'USER-1')).rejects.toThrow(BadRequestException);
    await expect(service.reserveStock('ITEM-1', 'PROD-1', 15, 'USER-1')).rejects.toThrow('STOCK_INSUFFICIENT');
  });
});
