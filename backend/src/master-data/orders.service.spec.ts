...
describe("OrdersService", () => {
  ...
  it("should block cancellation if order has passed into roasting phase", async () => {
    ...
    await expect(service.cancelOrder("ORD-1", "USER-1")).rejects.toThrow(
      ConflictException,
    )
    await expect(service.cancelOrder("ORD-1", "USER-1")).rejects.toThrow(
      "Cannot cancel order after roasting has started",
    )
  })
  ...
  it("should prove price list changes do not alter existing order prices", () => {
    ...
    expect(existingOrder.items[0].unitPrice).not.toBe(
      updatedPriceList.unitPrice,
    )
  })
})
