...
export class ProfitEngineService {
  ...
  async calculateProfitForPeriod(startDate: string, endDate: string): Promise<{
    ...
    try {
      ...
      const revenuePreVat = orders.reduce((sum, o) => sum + Number(o.preVatAmount), 0);
      const vatLiability = orders.reduce((sum, o) => sum + Number(o.vatAmount), 0);
      ...
      const payrolls = await queryRunner.manager
        ...
        .where('payroll.status IN (:...statuses)', { statuses: ['approved', 'paid'] })
        .andWhere('payroll.createdAt >= :startDate', { startDate })
        .andWhere('payroll.createdAt <= :endDate', { endDate })
        .getMany();

      const payrollExpenses = payrolls.reduce((sum, p) => sum + Number(p.totalAmount), 0);

      // 5. Final Profit Calculation
      const netProfit = revenuePreVat - productionCost - operatingExpenses - payrollExpenses;
