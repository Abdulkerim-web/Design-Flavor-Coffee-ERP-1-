...
export class PayrollService {
  ...
  async preparePayrollRun(
    periodStart: string,
    periodEnd: string,
    lines: { employeeUserId: string; baseSalaryAmount: number; advanceDeductionAmount: number; notes?: string }[],
    ...
    try {
      ...
      for (const l of lines) {
        ...
        if (l.baseSalaryAmount < 0 || l.advanceDeductionAmount < 0) {
          throw new BadRequestException('Salary and Deduction amounts must be positive');
        }

        const netAmount = Number(l.baseSalaryAmount) - Number(l.advanceDeductionAmount);
        
        if (netAmount < 0) {
          throw new BadRequestException(`Negative net salary not allowed for employee ${l.employeeUserId}`);
        }
