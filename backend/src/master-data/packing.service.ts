...
export class PackingService {
  ...
  async confirmPacking(
    ...
    try {
      ...
      if (orderItem.status !== 'ready-for-packing' && orderItem.status !== 'PACKING_IN_PROGRESS') {
        throw new BadRequestException('Order Item not in a valid state for packing.');
      }
      ...
      await queryRunner.commitTransaction();
      return { success: true, message: 'Packing confirmed and inventory deducted.' };
    } catch (err) {
