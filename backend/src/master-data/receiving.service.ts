...
export class ReceivingService {
  ...
  async performQC(
    ...
    try {
      const record = await queryRunner.manager.findOne(ReceivingRecord, { where: { id: recordId } });
      if (!record) throw new BadRequestException('Record not found');
      if (record.status !== 'received' && record.status !== 'qc-pending') {
        throw new BadRequestException(`Cannot perform QC in status ${record.status}`);
      }

      const total = acceptedQuantity + rejectedQuantity;
      if (total !== Number(record.receivedQuantity)) {
        throw new BadRequestException('Accepted and Rejected quantities must equal total received');
      }
  ...
  async approveShipment(
    ...
    try {
      ...
      if (record.status !== 'pending-manager-approval') {
        throw new BadRequestException(`Cannot approve in status ${record.status}`);
      }
      ...
      if (record.acceptedQuantity > 0) {
        // Generate LOT ID
        const origin = record.coffeeProduct.originId.substring(0, 3).toUpperCase();
        ...
        const lot = queryRunner.manager.create(Lot, {
          ...
          unitCostEtb,
          totalCostEtb: Number((record.acceptedQuantity * unitCostEtb).toFixed(2)),
        });
        ...
        stock.available = Number(stock.available) + Number(record.acceptedQuantity);
