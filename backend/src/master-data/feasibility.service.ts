...
export class FeasibilityEngineService {
   ...
   */
  computeGreenRequirement(roastedRequested: number, expectedYieldRatio: number): number {
  ...
  async checkFeasibility(
    ...
    try {
      ...
      const requirement = this.computeGreenRequirement(roastedQuantityRequested, expectedYieldRatio);
   ...
   */
  async reserveStock(orderItemId: string, coffeeProductId: string, quantityToReserve: number, userId: string) {
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect();
