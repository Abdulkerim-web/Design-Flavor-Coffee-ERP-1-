...
type ModuleId =
  | 'dashboard' | 'inventory' | 'suppliers' | 'quality' | 'production'
  | 'verification' | 'packaging' | 'orders' | 'delivery' | 'finance' | 'reports'
  | 'users' | 'audit' | 'settings' | 'portal'
  | 'customers' | 'payments' | 'banking' | 'expenses' | 'payroll'
  | 'approvals' | 'notifications' | 'design-system'
...
const QUICK_CREATE_OPTIONS = [
  ...
  { id: 'receive',  label: 'Record Green Bean Receiving',  icon: '📦', kbd: 'N R' },
...
function AppInner() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => { if (e.key === 'Escape') setQcOpen(false) }
  ...
  if (!currentUser) {
    if (unauthPage === 'team')  return <Team    onBack={() => setUnauthPage('welcome')} />
    if (unauthPage === 'login') return <Login />
    return <Welcome onSignIn={() => setUnauthPage('login')} onTeam={() => setUnauthPage('team')} />
  }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: 'var(--bg-primary)', overflow: 'hidden' }}>
      <Sidebar
        ...
        onNavigate={(id) => { setActive(id as ModuleId); if (isMobile) setSidebarOpen(false) }}
        ...
        {qcOpen && (
          ...
          >
            ...
            >
              ...
              <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border-neutral)', fontSize: 11, fontFamily: 'DM Mono', color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                Quick Create
              </div>
              {QUICK_CREATE_OPTIONS.map(opt => (
                ...
                >
                  ...
                  <span style={{ flex: 1, fontSize: 13.5, color: 'var(--text-primary)', fontFamily: 'Inter' }}>{opt.label}</span>
                  {opt.kbd && (
                    <span style={{ fontSize: 10.5, fontFamily: 'DM Mono', color: 'var(--text-muted)', background: 'var(--surface-02)', padding: '2px 6px', borderRadius: 4, border: '1px solid var(--border-neutral)' }}>{opt.kbd}</span>
                  )}
                </button>
              ))}
            </div>
      ...
      {isMobile && (
        <BottomNav
          active={active}
          onNavigate={(id) => { setActive(id as ModuleId); setSidebarOpen(false) }}
      ...
      {logoutOpen && (
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 17, fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '-0.01em', marginBottom: 6 }}>
              Sign out?
            </div>
            <p style={{ fontSize: 13.5, color: 'var(--text-secondary)', lineHeight: 1.5, margin: '0 0 22px' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                onClick={() => { setLogoutOpen(false); logout(); setUnauthPage('welcome') }}
                style={{
                  flex: 1, height: 40, borderRadius: 8,
                  border: 'none', background: 'var(--sem-danger)',
                  fontSize: 13.5, fontWeight: 600, fontFamily: 'Inter',
                  color: '#FFFFFF', cursor: 'pointer',
                  boxShadow: '0 3px 10px rgba(220,38,38,0.25)',
                  transition: 'all 0.15s ease',
                }}
                onMouseEnter={e => { const el = e.currentTarget; el.style.background = '#B91C1C'; el.style.boxShadow = '0 4px 14px rgba(220,38,38,0.35)' }}
                onMouseLeave={e => { const el = e.currentTarget; el.style.background = 'var(--sem-danger)'; el.style.boxShadow = '0 3px 10px rgba(220,38,38,0.25)' }}
              >
                Sign Out
              </button>
            </div>
      ...
      <CommandPalette
        ...
        onNavigate={(id) => { setActive(id as ModuleId); setSearchOpen(false) }}
      />
    </div>
  )
}
...
function ConnectionBanner() {
  ...
  return (
    <div style={{
      ...
      <span style={{ flex: 1, fontSize: 13, color: '#92400E', fontFamily: 'Inter', lineHeight: 1.4 }}>
        <strong>Connection lost.</strong> Some information may be unavailable until your connection is restored.
      </span>
      <button onClick={() => setDismissed(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#B45309', fontSize: 18, lineHeight: 1, padding: '0 4px', flexShrink: 0 }}>×</button>
    </div>
  )
}
