...
export type UIAction =
  ...
  | 'roasting.complete'
  | 'roasting.review-discrepancy'
  // Packing
  | 'packing.view'
  | 'packing.record'
  | 'packing.complete'
  | 'packing.review-discrepancy'
  // Delivery
  | 'delivery.view'
  | 'delivery.assign-driver'
  | 'delivery.start'
  | 'delivery.upload-proof'
  | 'delivery.verify'
  | 'delivery.customer-verify'
  | 'delivery.dispute'
  // Payments
  | 'payments.view'
  | 'payments.record'
  ...
  | 'settings.packaging.view'
  | 'settings.packaging.edit'
  | 'settings.expense-categories.view'
...
export function can(role: RoleId, action: UIAction): boolean {
  ...
  return !!(matrix as Record<string, Record<string, boolean>>)[moduleKey]?.[scope]
}
