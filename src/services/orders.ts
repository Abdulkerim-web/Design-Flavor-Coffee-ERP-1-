 ...
 */
import { apiRequest, mockRequest, safeRequest, type ListEnvelope, type PaginationParams, type SortParams } from './api'
import type { OrderStatusKey, FeasibilityKey, PaymentStatusKey } from '../lib/orderStatus'
...
export interface CreateOrderPayload {
  ...
  notes: string
  lines: Array<{
    coffeeType: string; origin: string; roastLevel: string; quantity: number; unit: 'KG'
  }>
  deliveryDate: string
  deliveryAddress: string
...
export async function listOrders(
  ...
  return safeRequest<ListEnvelope<Order>>(async () => {
    ...
    const mapped: Order[] = all.map(o => ({
      ...
      customer: { id: o.customerId, name: 'Customer Name', ref: 'CUS-REF', status: 'active' },
      ...
      payment: { status: 'unpaid', total: o.totalAmount ? `ETB ${o.totalAmount}` : "ETB 0.00", paid: "ETB 0.00", remaining: o.totalAmount ? `ETB ${o.totalAmount}` : "ETB 0.00" },
      createdAt: o.createdAt,
    }))

    const filtered = mapped.filter(o => {
      const q = (filters.search ?? '').toLowerCase()
      if (q && !o.ref.toLowerCase().includes(q) && !o.customer.name.toLowerCase().includes(q) && !o.customer.ref.toLowerCase().includes(q)) return false
...
export async function getOrderTimeline(orderId: string): Promise<TimelineEvent[]> {
  const MOCK: TimelineEvent[] = [
    { id: 1, event: 'Order created', actor: 'Hiwot Tadesse', timestamp: 'Aug 9, 2026 09:00 AM', state: 'completed' },
    { id: 2, event: 'Awaiting manager confirmation', timestamp: 'Aug 9, 2026', state: 'current' },
  ]
  void orderId
  return mockRequest(MOCK, 450)
}

/** Get a server-calculated pricing estimate for a new order. */
export async function getPricingEstimate(_payload: Partial<CreateOrderPayload>) {
  return safeRequest<PricingEstimate>(async () =>
    mockRequest({
      subtotal: 'ETB 97,500.00',
      vat:      'ETB 14,625.00',
      total:    'ETB 112,125.00',
      note:     'Estimate only. Final price confirmed upon order approval.',
    }, 800)
  )
}
...
export async function rejectOrder(orderId: string, reason: string, _managerId: string) {
  return safeRequest<{ success: boolean }>(async () => {
