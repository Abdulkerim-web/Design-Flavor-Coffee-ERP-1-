...
export const EntityHeader: FC<EntityHeaderProps> = ({
  eyebrow, title, subtitle, status, meta = [], actions = [], onBack, children, style,
}) => {
  ...
  return (
    <div style={{
      ...
      {onBack && isMobile && (
        <button
          ...
          style={{
            ...
            color: 'var(--text-secondary)', fontSize: 13, fontFamily: 'Inter',
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap' }}>
            <h1 style={{
              margin: 0, fontSize: isMobile ? 20 : 24, fontWeight: 700,
              color: 'var(--text-primary)', letterSpacing: '-0.02em',
              lineHeight: '30px', fontFamily: 'Inter, sans-serif',
            }}>
              {title}
            </h1>
            {status && (
          ...
          {subtitle && (
            <div style={{ fontSize: 14, color: 'var(--text-secondary)', fontFamily: 'Inter', marginTop: 3, lineHeight: '19px' }}>
              {subtitle}
            </div>
          )}
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            {visibleActions.map((a, i) => (
              <button
                ...
                style={{ height: 36, fontSize: 13, padding: '0 16px', gap: 6, opacity: a.disabled ? 0.5 : 1 }}
              >
                {a.icon && (
      ...
      {meta.length > 0 && (
        <div style={{
          display: 'flex', flexWrap: 'wrap', gap: '6px 20px', marginTop: 10,
          paddingTop: 10, borderTop: '1px solid var(--border-neutral)',
        }}>
          {meta.map((m, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'baseline', gap: 6, fontSize: 12.5, fontFamily: 'Inter' }}>
              <span style={{ color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{m.label}:</span>
              <span style={{ color: 'var(--text-secondary)', fontWeight: 500, whiteSpace: 'nowrap' }}>{m.value}</span>
            </div>
          ))}
        </div>
      )}
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
          {visibleActions.map((a, i) => (
            <button
              ...
              disabled={a.disabled}
              className={VARIANT_CLASS[a.variant ?? 'secondary']}
              style={{ height: 40, fontSize: 13, padding: '0 16px', gap: 6, flex: i === 0 && visibleActions.length === 1 ? 1 : undefined }}
            >
              {a.label}
            </button>
          ))}
