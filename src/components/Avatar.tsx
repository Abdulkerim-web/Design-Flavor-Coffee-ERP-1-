...
const SIZE: Record<AvatarSize, { box: number; font: number; dot: number; dotOffset: number; radius: number }> = {
...
export const Avatar: FC<AvatarProps> = ({
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-flex', flexShrink: 0, ...style }}>
      {src ? (
        <img
          ...
          style={{
            ...
            objectFit: 'cover', border: '2px solid var(--border-neutral)',
            display: 'block',
          }}
        />
      ) : (
        <div style={{
          ...
          fontFamily: 'Inter, sans-serif', border: '2px solid var(--border-neutral)',
      ...
      {status && (
        <span style={{
          position: 'absolute', bottom: s.dotOffset, right: s.dotOffset,
...
export const AvatarGroup: FC<AvatarGroupProps> = ({ users, max = 4, size = 'sm' }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center' }}>
      {shown.map((u, i) => (
        <div key={i} style={{ marginLeft: i > 0 ? -(s.box * 0.3) : 0, position: 'relative', zIndex: shown.length - i }}>
          <Avatar name={u.name} color={u.color} monogram={u.monogram} size={size}
            style={{ border: '2px solid var(--surface-01)', borderRadius: SIZE[size].radius + 2 }} />
        </div>
      ))}
      {hidden > 0 && (
        <div style={{
          ...
          width: s.box, height: s.box, borderRadius: s.radius,
          background: 'var(--surface-02)', border: '2px solid var(--border-neutral)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: s.font, fontWeight: 600, color: 'var(--text-secondary)',
