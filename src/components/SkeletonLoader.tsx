  ...
  <div style={{ padding: '28px 32px', display: 'flex', flexDirection: 'column', gap: 24 }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 16 }}>
      <div style={{ background: 'var(--surface-01)', border: '1px solid var(--border-neutral)', borderRadius: 10, overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-neutral)' }}>
          <Skeleton width={140} height={13} />
        </div>
        {[0,1,2,3,4].map(i => <SkeletonTableRow key={i} cols={4} />)}
      </div>
      <div style={{ background: 'var(--surface-01)', border: '1px solid var(--border-neutral)', borderRadius: 10, padding: 18, display: 'flex', flexDirection: 'column', gap: 16 }}>
