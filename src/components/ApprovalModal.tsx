...
export const ApprovalModal: FC<ApprovalModalProps> = ({
  ...
  const feasibilityPct = Math.min(
    100,
    Math.round((order.availableGreen / order.expectedGreen) * 100),
  )
      ...
      >
        ...
        >
          ...
          >
            {[
              ...
              ...(order.unitPrice
                ? [["Unit Price", formatETB(order.unitPrice)]]
                : []),
              ...(order.unitPrice
                ? [["Order Value", formatETB(order.quantity * order.unitPrice)]]
                : []),
            ].map(([k, v]) => (
              <div key={k}>
                <div
        ...
        >
          ...
          >
            <svg
              ...
              fill="none"
              stroke={
                isSufficient ? "var(--sem-success)" : "var(--sem-warning)"
              }
              strokeWidth="2"
              strokeLinecap="round"
            >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              {
                label: "Expected green required",
                value: formatKG(order.expectedGreen, 1),
                note: "incl. roasting yield loss",
              },
              {
                label: "Available in stock",
                value: formatKG(order.availableGreen, 1),
                note: isSufficient ? "sufficient" : "insufficient",
                warn: !isSufficient,
              },
              ...(!isSufficient
                ? [
                    {
                      label: "Shortfall",
                      value: formatKG(shortfall, 1),
                      note: "must be sourced or order reduced",
                      warn: true,
                    },
                  ]
                : []),
              ...
              >
                ...
                <span
                  style={{
                    ...
                    fontWeight: 600,
                    fontSize: 13,
                    color: (row as { warn?: boolean }).warn
                      ? "var(--sem-warning)"
                      : "var(--text-primary)",
                  }}
                >
          ...
          {!isSufficient && (
            ...
            >
              <strong>Risk:</strong> Confirming this order without sufficient
              stock may delay roasting and affect the delivery schedule.
            </div>
          )}
        </div>
        ...
        >
          <button
            ...
            onClick={onReject}
            disabled={loading}
            style={{ color: "var(--sem-danger)", height: 36, fontSize: 13 }}
          >
            Reject Order
          </button>
          <button
            className="btn-secondary"
            onClick={onWaitForStock}
            disabled={loading}
          ...
          <button
            ...
            style={{
              height: 36,
              fontSize: 13,
              background: isSufficient ? "var(--brand-primary)" : "#B45309",
              gap: 6,
            }}
          >
            {loading ? (
