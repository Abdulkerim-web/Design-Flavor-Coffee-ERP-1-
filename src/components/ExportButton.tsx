  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      <button
        ...
        onClick={() => setOpen(v => !v)}
        disabled={disabled}
        style={{ height: h, fontSize: fs, gap: 5, padding: '0 12px', opacity: disabled ? 0.5 : 1 }}
      >
      ...
      {open && (
        <div style={{
          ...
          <div style={{ padding: '8px 12px 5px', fontSize: 10, fontFamily: 'DM Mono', color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            Export Format
          </div>
          {formats.map(fmt => {
            ...
            return (
              ...
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: 2 }}>
                  <path d={cfg.icon} />
                </svg>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', fontFamily: 'Inter', lineHeight: '17px' }}>{cfg.label}</div>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'DM Mono', marginTop: 1 }}>{cfg.desc}</div>
                </div>
              </button>
            )
          })}
