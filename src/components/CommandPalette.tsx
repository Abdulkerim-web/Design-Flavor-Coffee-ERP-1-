...
interface EntityResult {
  id: string
  type: "customer" | "order" | "inventory" | "payment" | "delivery"
  label: string
  meta: string
  status?: string
  statusColor?: string
...
const ENTITY_DATABASE: EntityResult[] = [
  ...
  },
  {
    id: "c2",
    type: "customer",
    label: "Sunrise Café — Kazanchis",
    meta: "Branch · Addis Ababa",
    status: "Active",
    statusColor: "#16A34A",
  },
  {
  ...
  },
  {
    id: "c4",
    type: "customer",
    label: "Ethiopian Airlines Catering",
    meta: "Corporate · Bole International",
    status: "Overdue",
    statusColor: "#DC2626",
  },
  {
  ...
  // orders
  {
    ...
    type: "order",
    label: "#ORD-1042",
    meta: "Sunrise Café · Guji Medium, 50 KG",
    status: "Pending",
    statusColor: "#D97706",
  },
  {
    ...
    type: "order",
    label: "#ORD-1038",
    meta: "Hilton Addis Ababa · Limu Espresso, 25 KG",
    status: "Delivered",
    statusColor: "#16A34A",
  },
  {
    ...
    type: "order",
    label: "#ORD-4821",
    meta: "Hilton Addis Ababa · Yirgacheffe, 100 KG",
    status: "Processing",
    statusColor: "#2563EB",
  },
  {
    ...
    type: "order",
    label: "#ORD-1039",
    meta: "Bole Supermarket · Sidama Grade 1, 120 KG",
    status: "Approved",
    statusColor: "#16A34A",
  },
  // inventory
  ...
  },
  {
    id: "i2",
    type: "inventory",
    label: "Yirgacheffe Grade 1 · #GR-0291",
    meta: "120 KG on hand",
    status: "Quarantined",
    statusColor: "#DC2626",
  },
  ...
  // payments
  {
    ...
    type: "payment",
    label: "INV-2024-0819",
    meta: "Ethiopian Airlines · ETB 156,000",
    status: "Overdue",
    statusColor: "#DC2626",
  },
  {
    ...
    type: "payment",
    label: "INV-2024-0812",
    meta: "Hilton Addis Ababa · ETB 88,500",
    status: "Paid",
    statusColor: "#16A34A",
  },
  // deliveries
  {
    ...
    type: "delivery",
    label: "DEL-0291",
    meta: "ORD-1038 → Hilton Addis Ababa",
    status: "Completed",
    statusColor: "#16A34A",
  },
  {
    ...
    type: "delivery",
    label: "DEL-0292",
    meta: "ORD-1042 → Sunrise Café · Bole",
    status: "En route",
    statusColor: "#2563EB",
  },
]
...
const QUICK_ACTIONS = [
  ...
  {
    id: "qc-entry",
    label: "Create QC Inspection",
    icon: "M9 11l3 3L22 4M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11",
    kbd: "N Q",
  },
  {
    id: "new-batch",
    label: "Schedule Roasting Batch",
...
export default function CommandPalette({ open, onClose, onNavigate }: Props) {
  ...
  const runSearch = useCallback((q: string) => {
    ...
    searchTimer.current = setTimeout(() => {
      ...
      const found = ENTITY_DATABASE.filter(
        (e) =>
          e.label.toLowerCase().includes(lower) ||
          e.meta.toLowerCase().includes(lower),
      )
      setResults(found)
  ...
  if (searchState === "idle" || searchState === "loading") {
    RECENT_SEARCHES.forEach((r) =>
      flatItems.push({ section: "recent", label: r }),
    )
    idleActions.forEach((a) =>
      flatItems.push({ section: "action", id: a.id, label: a.label }),
    )
    idleModules.forEach((m) =>
      flatItems.push({ section: "module", id: m.id, label: m.label }),
    )
  ...
  const handleKey = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "Enter" && flatItems[cursor]) {
      ...
      if (item.section === "module" && item.id) {
        onNavigate(item.id)
        onClose()
      }
    }
        ...
        {icon && (
          <div
            style={{
              ...
              height: 26,
              borderRadius: 6,
              background: active ? "#E8EDE9" : "var(--surface-02)",
              display: "flex",
              alignItems: "center",
          ...
          >
            <SvgIcon
              d={icon}
              size={13}
              color={active ? "#2B4D3A" : "var(--text-secondary)"}
            />
          </div>
        )}
          ...
          <div
            style={{
              ...
              background: statusColor
                ...
                : "var(--surface-02)",
              border: `1px solid ${
                statusColor ? `${statusColor}40` : "var(--border-neutral)"
              }`,
              padding: "2px 7px",
              borderRadius: 999,
              flexShrink: 0,
  ...
  const SectionHead = ({ label, first }: { label: string first?: boolean }) => (
    <div
      style={{
        padding: `${first ? 10 : 14}px 16px 4px`,
        fontSize: 10,
        fontFamily: "DM Mono",
        color: "var(--text-muted)",
        letterSpacing: "0.1em",
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          width: isMobile ? "calc(100vw - 16px)" : 660,
          maxWidth: "90vw",
          background: "var(--surface-01)",
          border: "1px solid var(--border-neutral)",
          borderRadius: 14,
          boxShadow:
            "0 25px 50px -12px rgba(0,0,0,0.22), 0 0 0 1px rgba(0,0,0,0.04)",
          overflow: "hidden",
            ...
            <div>
                ...
                {RECENT_SEARCHES.map((r, i) => (
                  <button
                    ...
                    style={{
                      ...
                      textAlign: "left",
                      background:
                        cursor === i ? "var(--surface-02)" : "transparent",
                      cursor: "pointer",
                      borderLeft: `2px solid ${
                        cursor === i ? "#2B4D3A" : "transparent"
                      }`,
                    }}
                  >
                    <svg
              ...
              <div style={{ borderTop: "1px solid var(--border-neutral)" }}>
                ...
                {allActionItems.map((a) => {
                  void (rowIdx =
                    RECENT_SEARCHES.length + allActionItems.indexOf(a))
                  return (
                    <Row
                      key={a.id}
                      label={a.label}
