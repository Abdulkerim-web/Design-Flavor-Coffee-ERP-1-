...
/* ─── Status badge config ────────────────────────────────────── */
const STATUS_CONFIG: Record<PackingJobStatus, {
  label: string
  color: string
  bg: string
}> = {
  ...
  return (
    <div
      style={{
        ...
        height,
        background:
          "linear-gradient(90deg, var(--surface-01) 25%, var(--surface-02,#f3f4f6) 50%, var(--surface-01) 75%)",
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  background:
                    ev.state === "pending" ? "var(--surface-01)" : dotColor,
                  border: `2px solid ${dotColor}`,
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  animation: isCurrentPulsing
                    ? "timelinePulse 2s ease infinite"
                    : "none",
                }}
              >
                {ev.state === "completed" && (
...
function ConfirmPackingModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await submitPackingConfirmation(
        job.id,
        packedQtyInput,
        mats,
        notes || undefined,
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            PHP determines whether this submission is valid. Do not attempt to
            validate quantities here.
          </div>
          ...
          >
            ...
            <div>
              ...
              >
                Server determines authoritative count.
              </div>
            </div>
          </div>
        ...
        >
          ...
          <PrimaryButton
            ...
            disabled={submitting || !packedQtyInput.trim()}
          >
    ...
    >
      ...
      <div
        style={{
          fontSize: 12.5,
          color: "var(--text-secondary)",
          marginBottom: canReview && onReview ? 14 : 0,
        }}
      >
...
function MaterialsTable({
  ...
  if (materials.length === 0) {
    return (
      <div
        style={{ fontSize: 13, color: "var(--text-muted)", padding: "8px 0" }}
      >
    ...
    >
          ...
          return (
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: `1.5px solid ${
                  statusFilter === s ? cfg.color : "var(--border-neutral)"
                }`,
      ...
      >
        ...
        <select
          value={statusFilter}
          onChange={(e) =>
            setStatusFilter(e.target.value as PackingJobStatus | "")
          }
          style={{
            height: 38,
            borderRadius: 8,
              ...
              >
                {job.status === "in-progress"
                  ...
                  : job.status === "awaiting-manager" &&
                      can(role as any, "packing.review-discrepancy")
                    ? "Manager Review"
                    : "View Details"}
              </PrimaryButton>
            </Card>
  ...
  if (error || !job) {
    return (
      <div style={{ padding: 32, textAlign: "center" }}>
        <div style={{ color: "var(--sem-danger)", marginBottom: 12 }}>
          {error ?? "Job not found."}
        </div>
          ...
          <Card>
            ...
            >
              {([
                ...
                {
                  label: "Accepted Roasted Qty",
                  value: job.acceptedRoastedQty,
                },
                ...
                ...(job.startedAt
                  ...
                  : []),
                ...(job.packedAt
                  ? [{ label: "Packed At", value: job.packedAt }]
                  : []),
          ...
          <Card>
            ...
            {job.progressPercent !== undefined && (
              <div>
                ...
                >
                  ...
                  >
                    {job.packedQty} packed of {job.requiredPackedQty}
                  </span>
                  <span
                    style={{
                      fontSize: 12,
                      ...
                      fontWeight: 700,
                      color:
                        parseFloat(job.progressPercent) >= 100
                          ? "var(--sem-success)"
                          : "var(--sem-info)",
                    }}
                  >
                ...
                >
                  <div
                    style={{
                      height: "100%",
                      width: `${Math.min(100, parseFloat(job.progressPercent))}%`,
                      borderRadius: 4,
                      background:
                        parseFloat(job.progressPercent) >= 100
                          ? "var(--sem-success)"
            ...
            {job.packagingType && (
              <div style={{ display: "flex", gap: 20, marginTop: 12 }}>
                <div>
                  <Eyebrow>Packaging Type</Eyebrow>
          ...
          <div>
            {job.status === "pending" && canRecord && (
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                ...
                {actionError && (
                  <span style={{ fontSize: 12.5, color: "var(--sem-danger)" }}>
                    {actionError}
                  </span>
                )}
              </div>
            ...
            {job.status === "awaiting-manager" && (
              ...
              >
                ...
                <span
                  style={{ fontSize: 13.5, color: "#7C3AED", fontWeight: 600 }}
                >
              ...
              >
                ...
                >
                  {([
                    ...
                    ...(job.managerName
                      ? [{ label: "Confirmed By", value: job.managerName }]
                      : []),
                    ...(job.managerConfirmedAt
                      ? [
                          {
                            label: "Confirmed At",
                            value: job.managerConfirmedAt,
                          },
                        ]
                      : []),
                  ] as { label: string value: string }[]).map((item) => (
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {/* Discrepancy */}
        {hasDiscrepancy && <DiscrepancyPanel job={job} canReview={false} />}
              ...
              {hasDiscrepancy ? (
                <>
                  <PrimaryButton
                    ...
                    disabled={submitting}
                  >
                    {submitting && reviewDecision === "approve"
                      ? "Approving…"
                      : "Approve & Confirm Packing"}
                  </PrimaryButton>
                  <button
                  ...
                  >
                    {submitting && reviewDecision === "reject"
                      ? "Rejecting…"
                      : "Reject — Send Back"}
                  </button>
                </>
